...
public class MembershipFunctions
{
    ...
    public MembershipFunctions(ILogger<MembershipFunctions> logger, IMembershipService membershipService, IPlayerService playerService)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetMyMemberships(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "me/memberships")] HttpRequest req,
    ...
    )
    {
        try
        {
            ...
            _logger.LogInformation("Found {Count} memberships for player {PlayerId}", membershipInfos.Count, player.Id);

            return new OkObjectResult(new
            {
                memberships = membershipInfos,
                count = membershipInfos.Count
            });
        }
        catch (Exception ex)
        {
