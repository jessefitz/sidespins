...
public class AdminFunctions
{
    ...
    public async Task<HttpResponseData> AddPlayerToTeam(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "teams/{teamId}/members")] HttpRequestData req,
        string teamId)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            return await CreateErrorResponse(req, HttpStatusCode.InternalServerError, "Failed to add player");
        }
    }
    ...
    public async Task<HttpResponseData> RemovePlayerFromTeam(
        [HttpTrigger(AuthorizationLevel.Anonymous, "delete", Route = "teams/{teamId}/members/{playerId}")] HttpRequestData req,
        string teamId,
        string playerId)
    {
        try
        ...
        catch (Exception ex)
        {
            ...
            return await CreateErrorResponse(req, HttpStatusCode.InternalServerError, "Failed to remove player");
        }
    }
    ...
    public async Task<HttpResponseData> ChangePlayerRole(
        [HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = "teams/{teamId}/members/{playerId}/role")] HttpRequestData req,
        string teamId,
        string playerId)
    {
        try
        ...
        {
            _logger.LogError(ex, "Error changing role for player {PlayerId} in team {TeamId}", playerId, teamId);
            return await CreateErrorResponse(req, HttpStatusCode.InternalServerError, "Failed to change player role");
        }
    }
    ...
    public async Task<HttpResponseData> GetTeamMembers(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "teams/{teamId}/members")] HttpRequestData req,
        string teamId)
    {
        try
        {
            ...
            await response.WriteAsJsonAsync(new[]
            {
                new { userId = "user1", playerName = "John Doe", role = "player" },
                new { userId = "user2", playerName = "Jane Smith", role = "manager" },
                new { userId = "user3", playerName = "Admin User", role = "admin" }
            });
            return response;
        }
        catch (Exception ex)
        {
            ...
            return await CreateErrorResponse(req, HttpStatusCode.InternalServerError, "Failed to get team members");
        }
    }
