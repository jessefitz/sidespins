...
var enableCodeCors = (Environment.GetEnvironmentVariable("ENABLE_CODE_CORS") ?? "false").Equals(
    "true",
...
builder.Services.AddScoped<IMembershipService, CosmosMembershipService>(serviceProvider =>
{
    ...
    var databaseName = Environment.GetEnvironmentVariable("COSMOS_DB") ?? "sidespins";
    var containerName =
        Environment.GetEnvironmentVariable("COSMOS_MEMBERSHIPS_CONTAINER") ?? "TeamMemberships";

    return new CosmosMembershipService(
        cosmosClient,
        playerService,
        logger,
        databaseName,
